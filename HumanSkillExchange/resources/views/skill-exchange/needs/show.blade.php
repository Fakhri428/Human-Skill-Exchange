@component('layouts.public', ['title' => $need->title.' - Human Skill Exchange'])
        ...
        <section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div class="space-y-6">
                <article class="rounded-lg border border-slate-200 bg-white p-6">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <h1 class="mt-4 text-3xl font-semibold text-slate-950">{{ $need->title }}</h1>
                            <p class="mt-3 max-w-3xl text-base leading-7 text-slate-600">{{ $need->description }}</p>
                        </div>
                        @guest
                    ...
                    <div class="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
                        <p class="text-xs font-semibold uppercase tracking-normal text-slate-500">Barter dari pemilik need</p>
                ...
                <section id="exchange-form" class="scroll-mt-24 rounded-lg border border-slate-200 bg-white p-6">
                    <h2 class="text-lg font-semibold text-slate-950">Tawarkan bantuan</h2>
                    @guest
                        <p class="mt-3 text-sm leading-6 text-slate-600">Login untuk mengirim request ke pemilik need ini.</p>
                    ...
                    @else
                        @if (auth()->id() === $need->user_id)
                            <p class="mt-3 text-sm leading-6 text-slate-600">Ini need milik Anda sendiri. Untuk mencoba fitur exchange, buka need milik user lain dari halaman market.</p>
                        ...
                        @elseif ($myOffers->isEmpty())
                            <p class="mt-3 text-sm leading-6 text-slate-600">Buat offer terlebih dahulu agar partner tahu kontribusi apa yang Anda berikan.</p>
                        ...
                        @else
                            <form method="POST" action="{{ route('needs.request', $need) }}" class="mt-4 grid gap-4 md:grid-cols-[minmax(0,260px)_minmax(0,1fr)_auto] md:items-end">
                                ...
                                <div>
                                    ...
                                    <select id="offer_id" name="offer_id" required class="mt-2 w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500">
                                ...
                                <div>
                                    ...
                                    <textarea id="message" name="message" rows="3" required class="mt-2 w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500">Halo, saya bisa membantu need ini. Saya ingin mengajukan exchange dengan offer yang saya punya.</textarea>
                                </div>
                ...
                <section class="rounded-lg border border-slate-200 bg-white">
                    <div class="border-b border-slate-200 px-6 py-4">
                        <h2 class="text-lg font-semibold text-slate-950">Offer yang cocok</h2>
                    </div>
                            ...
                            <article class="p-6">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <a href="{{ route('offers.show', $offer) }}" class="font-semibold text-slate-950 hover:text-teal-700">{{ $offer->title }}</a>
                                        <p class="mt-1 text-xs text-slate-500">{{ $offer->user->name }} · {{ $offer->available_duration ?? 'Fleksibel' }}</p>
                                        <p class="mt-3 text-sm leading-6 text-slate-600">{{ $offer->description }}</p>
                                    </div>
                                </div>
                            </article>
                        @empty
                            <div class="p-6 text-sm text-slate-500">Belum ada offer dengan kategori yang sama.</div>
                        @endforelse
                    </div>
                </section>
            </div>
            ...
            <aside class="space-y-6">
                <section class="rounded-lg border border-slate-200 bg-white p-5">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <h2 class="truncate text-base font-semibold text-slate-950">{{ $need->user->name }}</h2>
                            <p class="truncate text-sm text-slate-500">{{ $need->user->profile?->location ?? 'Lokasi fleksibel' }}</p>
                        </div>
                    </div>
                    <p class="mt-4 text-sm leading-6 text-slate-600">{{ $need->user->profile?->bio ?? 'Member Human Skill Exchange' }}</p>
                ...
                <section class="rounded-lg border border-slate-200 bg-white p-5">
                    <h2 class="text-base font-semibold text-slate-950">Skill pemilik need</h2>
                ...
                <section class="rounded-lg border border-slate-200 bg-white p-5">
                    <h2 class="text-base font-semibold text-slate-950">Need lain</h2>
                    <div class="mt-4 space-y-3">
                        @forelse ($otherNeeds as $otherNeed)
                            <a href="{{ route('needs.show', $otherNeed) }}" class="block rounded-lg border border-slate-200 p-4 hover:bg-slate-50">
                                <p class="text-sm font-semibold text-slate-950">{{ $otherNeed->title }}</p>
                                <p class="mt-1 text-xs text-slate-500">{{ $otherNeed->category }}</p>
                            </a>
                        @empty
                            <p class="text-sm text-slate-500">Belum ada need lain.</p>
                        @endforelse
                    </div>
                </section>
            </aside>
